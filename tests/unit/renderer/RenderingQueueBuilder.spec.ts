...
export class RenderingQueueBuilderFixture {
	...
	public render_scriptPagesTrue_folderPagesTrue() {
		...
		const builder = new RenderingQueueBuilder(project.root, {scriptPages: true, folderPages: true});
		...
		Expect(pages[0].resource.name).toBe("my_folder");
	...
	public render_scriptPagesFalse_folderPagesFalse() {
		const project = this._createProject();
		const builder = new RenderingQueueBuilder(project.root, {scriptPages: false, folderPages: false});
		const queue = builder.build();
	...
	public render_scriptPagesTrue_folderPagesFalse() {
		const project = this._createProject();
		const builder = new RenderingQueueBuilder(project.root, {scriptPages: true, folderPages: false});
		const queue = builder.build();
	...
	public render_scriptPagesFalse_folderPagesTrue() {
		const project = this._createProject();
		const builder = new RenderingQueueBuilder(project.root, {scriptPages: false, folderPages: true});
		const queue = builder.build();
